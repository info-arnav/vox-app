...
const readOfficeDocumentText = async (filePath) => {
  const officeParser = await getOfficeParser()
  const parsed = await officeParser.parseOffice(filePath, {
    outputErrorToConsole: false
  })

  return String(typeof parsed?.toText === 'function' ? parsed.toText() : '').trim()
}
...
const createWordParagraph = (text, block, theme) => {
  ...
  return new Paragraph({
    ...
    bullet: isBullet ? { level: Math.max(0, Math.min(block.level - 1, 6)) } : undefined,
...
export const createWordDocument = async (payload) => {
  ...
  let existingText = ''
  let blockPayload = { ...safePayload }

  if (appendMode) {
    try {
      await fs.access(targetPath)
      existingText = await readOfficeDocumentText(targetPath)
    } catch (error) {
    ...
    if (existingText) {
      if (hasProvidedBlocks) {
        ...
        blockPayload = { ...blockPayload, content: mergedContent }
      }
    }
  }
  ...
  if (title) {
    documentParagraphs.push(
      new Paragraph({
        ...
        spacing: { before: toDocxTwips(0), after: toDocxTwips(12) },
        children: [
          new TextRun({
            text: title,
  ...
  if (documentParagraphs.length === 0) {
    documentParagraphs.push(new Paragraph({ children: [new TextRun('')] }))
  }

  const document = new Document({ sections: [{ children: documentParagraphs }] })
